...
data.temp <- Data[[Type[4]]]

groups <- factor(c(rep("A", 40), rep("Z", 40)))
...
data.train <- data.temp[rvec < x,]
data.test <- data.temp[rvec >= x,]
...
data.lda <- cbind(data.lda, predict(model)$x)
boxplot(LD1 ~ group, data=data.lda)
...
matplot(x = Data$Wavenumber_min_max,
                y = loadings[,1],
                type = "l", 
                main=paste("Loadings"),
                xlab = "Wavenumber [1/cm]",
                ...
                #xaxt ="n",
                xaxs = "i",
                xlim = c(max(Data$Wavenumber_min_max),min(Data$Wavenumber_min_max)))
...
confusionMatrix(groups.predict[["class"]], groups.test, positive = "Z")
