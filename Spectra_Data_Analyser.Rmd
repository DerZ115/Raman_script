...
groups <- c("A", "E","V", "Z")                # Filenames (unique part)
ending <- ".TXT"                    # Filetype
legend <- c("4h (control)", "12h (control)","4h (not induced)", "12h (6h induced)")    # Naming in publication
...
PCA <- prcomp(na.omit(Data[[paste(Type[2],"_outl", sep="")]][!is.na(Data$Groups),Sample]),
              retx = retx,
              center = center,
              rank. = rank,
              tol = tol,
...
group <- factor(c(rep("A", 40), rep("E", 40), rep("V", 40), rep("Z", 40)))
data.lda <- cbind(group, data.lda)
N <- nrow(data.lda)
...
data.lda.train <- data.lda[rvec < 0.75,]
data.lda.test <- data.lda[rvec >= 0.75,]

# Check the number of observations
nrow(data.lda.train)


model <- lda(group~., data = data.lda.train, )
model

lda.data <- cbind(data.lda.train, predict(model)$x)
boxplot(LD1 ~ group, data=lda.data)
