...
## Global parameters:
```{r User Settings}
work_dir <- ("C:/Users/Daniel/Dropbox/ubuntu_shared/Masterarbeit/Spectra_Data_Analyser/")     # Working directory (name of project folder)

data_folder <- "Data_VR/Data_G2/"             # Name of your Data folder in your working directory
groups <- c("E", "Z")                # Filenames (unique part)
ending <- ".TXT"                    # Filetype
legend <- c("12h (control)", "12h (6h induced)")    # Naming in publication
...
bc_pea_rw <- tryCatch({
  baseline(spectra = as.matrix(Data$Spectra_min_max), 
  ...
  error = function(e){
    print("Simultaneous Peak Detection and Baseline Correction failed.")
    return(NA)
  })
```
...
for (Stats in 1:length(Type)) {
  PCA <- tryCatch({
    prcomp(Data[[Type[Stats]]][!is.na(Data$Groups),Sample],
                retx = retx,
                center = center,
                rank. = rank,
                tol = tol,
...
data.temp <- Data[[Type[4]]] # Detrend: 2, FillPeaks: 4
factors <- apply(data.temp, 1, function(x){sqrt(sum(x^2))})
data.temp.norm <- sweep(data.temp, 1, factors, "/")

groups <- factor(c(rep("control", 40), rep("induced", 40)))

data.norm.plot <- as.data.frame(t(rbind(Data$Wavenumber_min_max, 
                                        apply(data.temp.norm[groups == "control",], 2, mean), 
                                        apply(data.temp.norm[groups == "induced",], 2, mean))))

rownames(data.norm.plot) <- c()
colnames(data.norm.plot) <- c("wavenumber", "control", "induced")

data.norm.plot.sd <- as.data.frame(t(rbind(Data$Wavenumber_min_max, 
                                        apply(data.temp.norm[groups == "control",], 2, sd), 
                                        apply(data.temp.norm[groups == "induced",], 2, sd))))

rownames(data.norm.plot.sd) <- c()
colnames(data.norm.plot.sd) <- c("wavenumber", "control", "induced")

data.norm.plot.low <- data.norm.plot - data.norm.plot.sd

data.norm.plot.high <- data.norm.plot + data.norm.plot.sd

ggplot(data=data.norm.plot, aes(x=wavenumber)) + 
  geom_line(aes(y=control, col="control")) + 
  geom_line(aes(y=induced, col="induced")) + 
  geom_ribbon(aes(ymin=data.norm.plot.low$control, ymax=data.norm.plot.high$control, fill="control"), linetype=2, alpha=0.2, show.legend=F) + 
  geom_ribbon(aes(ymin=data.norm.plot.low$induced, ymax=data.norm.plot.high$induced, fill="induced"), linetype=2, alpha=0.2, show.legend=F) +
  coord_cartesian(expand=F) + scale_x_reverse(breaks=seq(300, 1600, 100), minor_breaks=seq(300, 1600, 50)) + ylab("intensity")
  
  

# , rep("V", 40), rep("Z", 40)

N <- nrow(data.temp.norm)
...
data.train <- data.temp.norm[rvec < x,]
data.test <- data.temp.norm[rvec >= x,]
groups.train <- groups[rvec < x]
groups.test <- groups[rvec >= x]

nrow(data.train)


PCA <- prcomp(na.omit(data.train),
              retx = retx,
              center = TRUE,
              rank = 5,
              tol = tol,
...
scores$group <- groups.train

str(scores)

ggplot(scores, aes(x=PC1, y=PC2, col=group)) + geom_point()

loadings.PCA <- PCA$rotation

loadings.plot <- as.data.frame(cbind(Data$Wavenumber_min_max, loadings.PCA))
colnames(loadings.plot)[1] <- "wavenumber"

ggplot(data=loadings.plot, aes(x=wavenumber)) + geom_line(aes(y=PC1, col="PC1")) + geom_line(aes(y=PC5, col="PC5")) + coord_cartesian(expand=FALSE) + scale_x_reverse() + geom_hline(yintercept=0)
      
ls(PCA)
data.lda <- as.data.frame(PCA$x)



data.lda <- cbind(groups.train, data.lda)

colnames(data.lda)[1] <- "group"


model <- lda(group~., data = data.lda, )
model

data.lda.plot <- cbind(data.lda[1], predict(model)$x)
boxplot(LD1 ~ group, data=data.lda.plot)

ggplot(data.lda.plot, aes(x=group, y=LD1, fill=group)) + geom_violin()

#ggplot(data.lda, aes(LD1, LD2)) +
#  geom_point(aes(color=group))


loadings.LDA <- model$scaling
loadings <- loadings.PCA %*% loadings.LDA

data.loadings.plot <- as.data.frame(cbind(Data$Wavenumber_min_max, loadings))
colnames(data.loadings.plot)[1] <- "Wavenumber"

ggplot(data.loadings.plot, aes(x=Wavenumber, y=LD1)) + geom_line() +
  coord_cartesian(expand=FALSE) + 
  scale_x_reverse(breaks=seq(300, 1600, 100), minor_breaks=seq(300, 1600, 50)) +
  geom_hline(yintercept = 0)

data.test.matrix <- as.matrix(data.test)

data.test.scores <- data.test.matrix %*% loadings.PCA

groups.predict <- predict(model, as.data.frame(data.test.scores))
groups.predict[[2]]

confusionMatrix(groups.predict[["class"]], groups.test, positive = "induced")
