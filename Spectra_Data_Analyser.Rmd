  ...
  if(groups[1] != Files[1]){
    ...
    for (i in 1:length(groups)){
      Pos <- c(grep(pattern = as.character(groups[i]), 
                    x = str.replace(Files, ending, ""))) # check for parts of filenames
      #print(" ")
      #print(Pos)
...
if (remove_area == T) {
  ...
  # adjust parameters here:
  bc_pea_area <- tryCatch({
    baseline(spectra = as.matrix(Data$Spectra_area), 
             method = "peakDetection", 
             left=60,    # Smallest window size for peak widths
             right=60,   # Largest window size for peak widths
             lwin=120,   # Smallest window size for minimums and medians in peak removed spectra
...
# Simultaneous Peak Detection and Baseline Correction - REDUCED WAVENUMBER/EXCLUDED AREA SPECTRA MEAN AND MEDIAN
if ((remove_area == T) && (!(is.na(bc_pea_area)))) {  
  # extract data from baseline object
...
if (select_outlier == T) {
  for (Stats in 1:length(Type)) {
    PCA <- tryCatch({
      prcomp(na.omit(Data[[paste(Type[Stats],"_outl", sep="")]][!is.na(Data$Groups),Sample]),
             retx = retx,
             center = center,
             rank. = rank,
             tol = tol,
